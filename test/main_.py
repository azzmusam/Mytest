...
if __name__ == '__main__':
    ...
    with open("configs/multiconfig.yaml", 'r') as stream:
        try:
    ...
    while i < maximum_time_steps:
        ...
        while (not done) and  i < maximum_time_steps:
            ...
            i +=1
            agent.upgrade()            
            if i==0 or i== maximum_time_steps:
                pdb.set_trace()
                try:
                    observation, average_train_times, average_train_time = env.reset(1)
                    train_travel_time.append(average_train_time)
                    print(train_travel_time)
