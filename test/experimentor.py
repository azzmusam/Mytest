    ...
    def tester(self):
        ...
        if self.modelnumber ==None:
            ...
            '''if self.index == 'individual':
                ...
                try:
                    filename = 'two_0.2_deepqnet.ckpt-0'
                    chkpt = os.path.join(*[path, 'tmp', 'two_intersection', 'q_eval', filename])
    ...
    def file_finder(self, data):
        ...
        if len(reward_files)!=0:
            ...
            '''for i in range(len(sorted_reward_files)):
                sorted_reward_files[i] = self.filefolder + '/' + 'result' + str(sorted_reward_files[i]) + '.csv' '''
